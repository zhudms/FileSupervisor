...
import com.example.explorer.util.FileCategoryHelper.FileCategory;
...
public class FileIconLoader implements Callback {
    ...
    public long getDbId(String path, boolean isVideo) {
        ...
        Log.d(TAG, "path=" + path + " " +uri);
        
        /* 为了解决，有些路径是dcim，有些是DCIM ,所以分别查找*/
        String selection = FileColumns.DATA + "=?" + " OR " + FileColumns.DATA + "=?";
        
        String path1 = path.replace("DCIM", "dcim");
        String path2 = path.replace("dcim", "DCIM");
